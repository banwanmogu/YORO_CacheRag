...
本项目实现了一个带有语义缓存（LRU + embedding 相似度）和检索增强生成（RAG）的轻量级流水线。下面描述了系统的核心组件与调用流程（pipeline）：

### 概览

- 数据采集与分割：使用 `WebBaseLoader` 抓取网页内容，并用 `RecursiveCharacterTextSplitter` 将长文档切成若干语义块（chunk），每个 chunk 为检索最小单位。
- 向量化与索引：对 chunk 进行嵌入（`OpenAIEmbeddings`），并存入 `InMemoryVectorStore`（项目中可替换为 FAISS/Annoy/其他持久化向量库）。
...
- Dense 重排（可选）：在 BM25/union 候选上使用 bi-encoder / cross-encoder 对候选做 rerank，提高语义匹配质量。（示例可选实现：sentence-transformers 的 `CrossEncoder` 或 `SentenceTransformer`）
...
2. Agent 调用 `retrieve_context(query)`：
   ...
   - 对 candidate pool 进行 Dense 重排（可选）：
     - 若使用 bi-encoder：对 query 与每个 candidate 的 embedding 计算相似度并排序（快速、可批处理）。
     - 若使用 cross-encoder：对 (query, candidate) pair 逐一评分并排序（更慢但通常更准确，推荐对较小候选集使用）。
   - 取重排后 top-K（例如 5~10）结果序列化为上下文，写入 `SemanticCache`，返回给 Agent。
3. Agent 将检索到的上下文与系统 prompt 一起传给 LLM（eg. `gpt-4o-mini`），并按 `ResponseFormat` 返回结构化答案。
...
- Agent 逻辑：LangGraph / LangChain 的 `create_agent` 与自定义 Tools（`retrieve_context`, `search_web`）。
...
- 成本与吞吐：若使用 OpenAI 嵌入/LLM，请考虑速率限制与成本；可把 heavy rerank 放在本地 `sentence-transformers` 模型或用更小模型做裁剪。

---

如需我把 `retrieve_context` 中的 dense reranker 实现补上并在仓库中添加示例文件（`rerank_dense.py`）并做一次 smoke-test，我可以继续实现并运行（会包含对 `requirements.txt` 的建议依赖修改）。
